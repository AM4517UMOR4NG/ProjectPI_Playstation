...
<section class="mb-5">
  <div class="d-flex justify-content-between align-items-center mb-4 section-header">
    <h3 class="text-white fw-bold m-0">
      ...
      Unit PlayStation Tersedia
    </h3>
    <a href="{{ route('pelanggan.unitps.list') }}" class="btn btn-cta">
            ...
            <div class="dash-frame">
              @if($unit->foto)
                <img src="{{ str_starts_with($unit->foto, 'http') ? $unit->foto : asset('storage/' . $unit->foto) }}" 
                     alt="{{ $unit->name }}" class="img-fluid">
              @else
                <img src="https://placehold.co/300x200/23284a/60a5fa?text={{ urlencode($unit->model) }}" alt="{{ $unit->name }}" class="img-fluid">
              @endif
            </div>
            <div class="card-content">
              <div class="p-2 flex-grow-1">
                <h5 class="mb-2 text-center text-white fw-bold">{{ $unit->name }}</h5>
                <div class="text-center text-light mb-2">{{ $unit->model }} - {{ $unit->brand }}</div>
                <div class="fw-bold text-price mb-2 text-center mt-2">
                  Rp {{ number_format($unit->price_per_hour, 0, ',', '.') }}/jam
                </div>
                <div class="text-center">
                  <span class="badge-stock">
                    <i class="bi bi-box-seam me-1"></i>Stok: {{ $unit->stock ?? 0 }}
                  </span>
                </div>
              </div>
              <div class="pt-3 mt-auto">
                <a href="{{ route('pelanggan.rentals.create') }}?type=unitps&id={{ $unit->id }}" class="btn btn-cta w-100">
                  <i class="bi bi-cart-plus me-2"></i>Sewa Unit
                </a>
              </div>
            </div>
          </div>
  ...
  @else
    <div class="col-12 text-center py-5">
      <div class="text-muted">
        ...
        <p class="fs-5">Tidak ada unit PlayStation tersedia saat ini.</p>
      </div>
    </div>
  @endif
</section>
