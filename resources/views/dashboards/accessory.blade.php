...
<div class="dash-hero">
  <h2>Sewa Aksesoris PlayStation</h2>
  <p>Temukan aksesoris pelengkap untuk pengalaman bermain terbaik!</p>
</div>
...
<section class="mb-5">
  <div class="d-flex justify-content-between align-items-center mb-4 section-header">
    ...
    <a href="{{ route('pelanggan.accessories.list') }}" class="btn btn-cta">
            ...
            <div class="dash-frame">
              @if($acc->gambar)
                <img src="{{ str_starts_with($acc->gambar, 'http') ? $acc->gambar : asset('storage/' . $acc->gambar) }}" 
                     alt="{{ $acc->nama }}" class="img-fluid">
              @else
                <img src="https://placehold.co/300x200/23284a/60a5fa?text={{ urlencode($acc->nama) }}" alt="{{ $acc->nama }}" class="img-fluid">
              @endif
            </div>
            <div class="card-content">
              <div class="p-2 flex-grow-1">
                <h5 class="mb-2 text-center text-white fw-bold">{{ $acc->nama }}</h5>
                <div class="fw-bold text-price mb-2 text-center mt-2">
                  Rp {{ number_format($acc->harga_per_hari, 0, ',', '.') }}/hari
                </div>
                <div class="text-center">
                  <span class="badge-stock">
                    <i class="bi bi-box-seam me-1"></i>Stok: {{ $acc->stok }}
                  </span>
                </div>
              </div>
              <div class="pt-3 mt-auto">
                <a href="{{ route('pelanggan.rentals.create') }}?type=accessory&id={{ $acc->id }}" class="btn btn-cta w-100">
  ...
  @else
    <div class="col-12 text-center py-5">
      <div class="text-muted">
        ...
        <p class="fs-5">Tidak ada aksesoris tersedia saat ini.</p>
      </div>
    </div>
  @endif
</section>
